use daemon_console::{AsyncCommandHandler, TerminalApp, get_info, AppAction};
...
impl AsyncCommandHandler for RegisterCommand {
    async fn execute_async(&mut self, app: &mut TerminalApp, args: &[&str]) -> String {
        if args.len() < 2 {
            return get_info!("Usage: register <command> <reply>");
        }
        ...
        // 创建一个新的命令处理器
        let handler = Box::new(move |_app: &mut TerminalApp, _args: &[&str]| -> String {
            get_info!(&reply)
        });
        ...
        if let Some(sender) = &app.action_sender {
            ...
            get_info!(&format!("Command '{}' registered successfully!", cmd))
        } else {
            get_info!("Failed to register command: no action sender available")
        }
    }
